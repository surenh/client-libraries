...
def get_profile_for_url(apikey, apisecret, url)
  ...
  # Encode the URL for safe passage and ensure that the calculated signature is appended to the call along with the time stamp.
	request_path = '/content_profile?url=' + ERB::Util.url_encode(url) + '&' + apikey_string + '&' + ts_string + '&' + sig_string 
